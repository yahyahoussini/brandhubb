...
type BlogReadEvent = Database['public']['Tables']['analytics_events']['Row'];
...
interface BlogData {
  topPosts: (PostStat & { avgReadTime: number; scroll75Rate: number })[];
  totalViews: number;
  totalAssistedLeads: number;
  avgReadTime: number;
  ctaClicksByPost: Record<string, CtaClicks>;
}
  ...
  useEffect(() => {
    const fetchBlogAnalytics = async () => {
      try {
        ...
        const postStats: Record<string, PostStat> = {};
        ...
        const ctaClicksByPost: Record<string, CtaClicks> = {};
        blogCTAClicks?.forEach((cta) => {
          const slug = getStringProp(cta.props, 'post_slug');
          const type = getStringProp(
            cta.props,
            'cta_type',
            'unknown'
          ) as keyof CtaClicks;
        ...
        const topPosts = Object.values(postStats)
          .map((post) => ({
            ...post,
            avgReadTime:
              post.views > 0 ? Math.round(post.totalReadTime / post.views) : 0,
            scroll75Rate:
              post.views > 0 ? (post.scroll75Count / post.views) * 100 : 0,
          }))
        ...
        const avgReadTime =
          (blogReads?.length ?? 0) > 0
            ? Math.round(
                (blogReads as BlogReadEvent[]).reduce(
                  (sum, r) => sum + getNumberProp(r.props, 'read_time_sec', 0),
                  0
                ) / (blogReads?.length ?? 1)
              )
            : 0;
    ...
    void fetchBlogAnalytics();
  }, []);

  const totalCtaClicks = useMemo(() => {
    if (!data) return 0;
    return Object.values(data.ctaClicksByPost).reduce(
      (sum, ctas) => sum + ctas.inline + ctas.banner + ctas.footer,
      0
    );
  }, [data]);
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {`${Math.floor(data.avgReadTime / 60)}m ${
                data.avgReadTime % 60
              }s`}
            </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <span className="ml-1 font-medium">
                        {`${Math.floor(post.avgReadTime / 60)}m`}
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Scroll:</span>
                      <span className="ml-1 font-medium">
                        {`${post.scroll75Rate.toFixed(0)}%`}
                      </span>
                    </div>
                    <div>
              ...
              {Object.entries(data.ctaClicksByPost)
                .sort(
                  ([, a], [, b]) =>
                    ...
                    (b.banner ?? 0) +
                    (b.footer ?? 0) -
                    ((a.inline ?? 0) + (a.banner ?? 0) + (a.footer ?? 0))
                )
                .slice(0, 6)
                .map(([slug, ctas]) => {
                  const totalClicks =
                    (ctas.inline ?? 0) + (ctas.banner ?? 0) + (ctas.footer ?? 0);
                              ...
                              <div className="w-16 bg-gray-200 rounded-full h-1">
                                <div
                                  ...
                                  style={{
                                    width:
                                      totalClicks > 0
                                        ? `${((count as number) / totalClicks) * 100}%`
                                        : '0%',
                                  }}
                                />
                              </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Total Assisted Leads
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  {`${
                    data.totalViews > 0
                      ? (
                          (data.totalAssistedLeads / data.totalViews) *
                          100
                        ).toFixed(1)
                      : 0
                  }% conversion rate`}
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600">
                  {`${(
                    data.topPosts.reduce((sum, p) => sum + p.scroll75Rate, 0) /
                    Math.max(data.topPosts.length, 1)
                  ).toFixed(1)}%`}
                </div>
                <p className="text-sm text-muted-foreground">
                  Avg Scroll Depth
            ...
            <div className="mt-6 p-4 bg-purple-50 rounded-lg">
              ...
              <ul className="text-sm space-y-1 text-muted-foreground">
                <li>
                  • Focus on top-performing posts for promotion and internal
                  linking
                </li>
                <li>
                  • Improve CTA placement in posts with high views but low
                  conversion
                </li>
                <li>
                  • Analyze read time vs scroll depth to identify engagement
                  issues
                </li>
                <li>
                  • Create more content similar to your best-performing posts
                </li>
              </ul>
            </div>
          </CardContent>
